...
//获取短信验证码
 async verifycodesendAction(){
   ...
   let qianming = '猫oh';//this.setup.SMS_qianming;

   let temp_code;
   if(data.type ==1){
     temp_code = 'SMS_16300452';//this.setup.SMS_zhuce
   }
   let info = {
     'extend':data.mobile,
     'sms_type':'normal',
     'sms_free_sign_name':qianming,
     'sms_param':`{"code":"${code}","product":"${this.setup.SMS_product}","msg":"${code}"}`,
     'rec_num':data.mobile,
     'sms_template_code':temp_code
   }
   console.log('qianming==='+qianming);
